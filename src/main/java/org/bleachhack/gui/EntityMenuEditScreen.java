...
public class EntityMenuEditScreen extends WindowScreen {
	...
	public void onRenderWindow(DrawContext context, int window, int mouseX, int mouseY) {
		...
		if (window == 0) {
			...
			context.fill(seperator, y, seperator + 1, y + h, 0xff606090);

			context.drawTextWithShadow(MinecraftClient.getInstance().textRenderer, "Interactions:", x + 6, y + 5, 0xffffff);

			boolean mouseOverAdd = mouseX >= seperator - 16 && mouseX <= seperator - 3 && mouseY >= y + 3 && mouseY <= y + 15;
			Window.fill(context, seperator - 16, y + 3, seperator - 3, y + 15,
					mouseOverAdd ? 0x4fb070f0 : 0x60606090);
			context.drawTextWithShadow(MinecraftClient.getInstance().textRenderer, "\u00a7a+", seperator - 12, y + 5, 0xffffff);
			...
			if (scroll > 0) {
				...
				Window.fill(context, x + 3, y + 17, seperator - 2, y + 33,
			...
			if (interactions.size() - maxEntries > 0 && scroll < interactions.size() - maxEntries) {
				...
				Window.fill(context, x + 3, y + 17 + (maxEntries * 17), seperator - 2, y + 33 + (maxEntries * 17),
						mouseOver ? 0x4fb070f0 : 0x50606090);
				context.drawCenteredTextWithShadow(textRenderer,
			...
			for (String entry: interactions.getEntries()) {
				...
				Window.fill(context, x + 3, curY, seperator - 2, curY + 16,
			...
			if (selectedEntry != null) {
				context.drawTextWithShadow(textRenderer, "Name:", seperator + 8, y + 5, 0xffffff);

				editNameField.setX(seperator + 8);
				editNameField.setY(y + 18);
				editNameField.setWidth(w - (seperator - x) - 16);
				editNameField.render(context, mouseX, mouseY, client.getTickDelta());

				context.drawTextWithShadow(textRenderer, "Value:", seperator + 8, y + 45, 0xffffff);

				editValueField.setX(seperator + 8);
				editValueField.setY(y + 57);
				editValueField.setWidth(w - (seperator - x) - 16);
				editValueField.render(context, mouseX, mouseY, client.getTickDelta());
				...
				context.drawTextWithShadow(textRenderer, "Insert:", seperator + 8, y + 85, 0xffffff);
				...
				for (String insert: new String[] { "%name%", "%uuid%", "%health%", "%x%", "%y%", "%z%"}) {
					...
					context.fill(seperator + 7 + curX, y + 97 + line * 14, seperator + 10 + curX + textLen, y + 108 + line * 14, mouseOverInsert ? 0x9f6060b0 : 0x9f8070b0);
					context.drawTextWithShadow(textRenderer, insert, seperator + 9 + curX, y + 99 + line * 14, 0xffffff);
				...
				context.drawTextWithShadow(textRenderer, "Mode:", seperator + 8, y + 120 + line * 14, 0xffffff);
				...
				for (Pair<String, String> pair: new Pair[] { Pair.of("Normal", ""), Pair.of("Suggest", ">suggest "), Pair.of("Open Url", ">url ") }) {
					...
					context.fill(seperator + 7 + curX, startY + line * 14, seperator + 10 + curX + textLen, startY + 11 + line * 14, mouseOverInsert ? 0x9f6060b0 : 0x9f8070b0);
					context.drawTextWithShadow(textRenderer, pair.getLeft(), seperator + 9 + curX, startY + 2 + line * 14, 0xffffff);
				...
				Window.fill(context, x + w - 70, y + h - 22, x + w - 5, y + h - 4, 0x60e05050, 0x60c07070, mouseOverDelete ? 0x20e05050 : 0x10e07070);
				context.drawCenteredTextWithShadow(textRenderer, "Delete", x + w - 37, y + h - 17, 0xf0f0f0);
