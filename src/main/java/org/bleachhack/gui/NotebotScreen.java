...
public class NotebotScreen extends WindowScreen {
	...
	public void onRenderWindow(DrawContext context, int window, int mouseX, int mouseY) {
		...
		if (window == 0) {
			...
			context.drawCenteredTextWithShadow(textRenderer, "Page " + (page + 1), x + 55, y + 5, 0xc0c0ff);

			fillButton(context, x + 10, y + h - 13, x + 99, y + h - 3, 0xff3a3a3a, 0xff353535, mouseX, mouseY);
			context.drawCenteredTextWithShadow(textRenderer, "Download Songs..", x + 55, y + h - 12, 0xc0dfdf);
			...
			for (String s : files) {
				...
				fillButton(context, x + 5, y + 15 + c * 10, x + 105, y + 25 + c * 10,
				...
				context.drawCenteredTextWithShadow(textRenderer, textRenderer.trimToWidth(s, 100), x + 55, y + 16 + c * 10, -1);
			...
			if (entry != null) {
				int textX = x + w - w / 4;
				context.drawCenteredTextWithShadow(textRenderer, entry.name, textX, y + 8, 0xffffff);
				context.drawCenteredTextWithShadow(textRenderer, "By: " + entry.author, textX, y + 18, 0xb0b0b0);
				
				context.drawCenteredTextWithShadow(textRenderer, "Format: \u00a7a" + entry.format, textX, y + 35, 0xb0b0b0);
				context.drawCenteredTextWithShadow(textRenderer, "Length: \u00a7f" +  entry.length / 20 + "s", textX, y + 45, 0xb0b0b0);
				//drawCenteredText(matrices, textRenderer, "Notes: \u00a7f" + entry.notes.size(), textX, y + 55, 0xb0b0b0);
				context.drawCenteredTextWithShadow(textRenderer, "Noteblocks: ", textX, y + 62, 0x80f080);
				...
				for (Entry<Instrument, ItemStack> e : NotebotUtils.INSTRUMENT_TO_ITEM.entrySet()) {
					...
					if (count != 0) {
						...
						context.drawCenteredTextWithShadow(textRenderer, StringUtils.capitalize(e.getKey().asString()) + " x" + count,
								textX, y + 74 + c2 * 10, 0x50f050);

						DiffuseLighting.enableGuiDepthLighting();
						context.drawItem(e.getValue(), textX + 55, y + 70 + c2 * 10);
				...
				fillButton(context, x + w - w / 2 + 10, y + h - 15, x + w - w / 4, y + h - 5, 0xff903030, 0xff802020, mouseX, mouseY);
				fillButton(context, x + w - w / 4 + 5, y + h - 15, x + w - 5, y + h - 5, 0xff308030, 0xff207020, mouseX, mouseY);
				fillButton(context, x + w - w / 4 - w / 8, y + h - 27, x + w - w / 4 + w / 8, y + h - 17, 0xff303080, 0xff202070, mouseX, mouseY);

				int pixels = (int) Math.round(MathHelper.clamp((w / 4d) * ((double) playTick / (double) entry.length), 0, w / 4d));
				context.fill(x + w - w / 4 - w / 8, y + h - 27, (x + w - w / 4 - w / 8) + pixels, y + h - 17, 0x507050ff);

				context.drawCenteredTextWithShadow(textRenderer, "Delete", (int) (x + w - w / 2.8), y + h - 14, 0xff0000);
				context.drawCenteredTextWithShadow(textRenderer, "Select", x + w - w / 8, y + h - 14, 0x00ff00);
				context.drawCenteredTextWithShadow(textRenderer, playing ? "Previewing.." : "Preview", x + w - w / 4, y + h - 26, 0x6060ff);
			}
		}
	}
	...
	private void fillButton(DrawContext context, int x1, int y1, int x2, int y2, int color, int colorHover, int mouseX, int mouseY) {
		context.fill(x1, y1, x2, y2, (mouseX > x1 && mouseX < x2 && mouseY > y1 && mouseY < y2 ? colorHover : color));
	}
}
